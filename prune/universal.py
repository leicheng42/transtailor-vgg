...
class IterRecoverFramework():
    ...
    def freeze_bn(self):
        ...
        for m in self.pack.net.modules():
            if isinstance(m, GatedBatchNorm2d):
                for p in m.parameters():
                    self._status[id(p)] = p.requires_grad
                    p.requires_grad = False
                #m.bn.weight.requires_grad = True
    def restore_bn(self):
        for m in self.pack.net.modules():
            if isinstance(m, GatedBatchNorm2d):
                for p in m.parameters():
    ...
    def tock(self, lr_min=0.001, lr_max=0.01, tock_epoch = 20, mute=False, acc_step=1):
        ...
        def iter_hook(curr_iter, total_iter):
            ...
            ignored_params = list(map(id, self.pack.net.fc.parameters()))
            base_params = filter(lambda p: id(p) not in ignored_params, self.pack.net.parameters())
            self.pack.optimizer = torch.optim.SGD([{'params': base_params},{'params': self.pack.net.fc.parameters(), 'lr': 0.005}], lr=0.0005, momentum=0.9, weight_decay=0.0005)
